...
function Event() {
    ...
    useEffect(() => {
        fetch(' https://au-temps-donne-api.onrender.com/api/event/activites_prives')
            .then(response => {
                if (!response.ok) {
                    ...
                    <div className="box-container">
                        {activities.map((activity, index) => (
                            <div key={index} className="elm_box">
                                <div className="infos">
                                    <div className="info">
                                        <img height="180px" src={image} style={{ borderRadius: '25px' }} />
                                ...
                                <Link to={`/admin/activiteprv_details/${activity.ID_Activite}`} className="view">Voir plus<img className="voir-all-icon" height="15px" width="17px" src={viewIcon} alt="Voir tout" /></Link>
                            </div>
                        ))}
                    </div>
                </div>
